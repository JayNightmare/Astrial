  ...
  client.on("messageCreate", async (message) => {
    ...
    if (cmd.length === 0 && message.content === botTag) {
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setLabel("Invite Me").setStyle(ButtonStyle.Link).setURL(`${client.invite}`),
        new ButtonBuilder().setLabel("Support Server").setStyle(ButtonStyle.Link).setURL(`${client.config.ssLink}`),
      );

      const embed = new EmbedBuilder()
        .setColor(client.color)
        .setAuthor({ name: `Hey It's me ${client.user.username}`, iconURL: client.user.displayAvatarURL() })
        .setDescription(`Astrial is a powerful music bot for Discord that lets you stream high-quality music, request songs, and customize audio with filters. Join a voice channel and elevate your server's vibe!`)
        .addFields([
        ...
        return message.reply({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(`Hello, ${message.author}\nYou have Just discovered a [premium](https://discord.gg/kSwwUD8RDe) command, This command is only accessible to premium only members. Upgrade to Premium to Access Exclusive Features and commands, check it out!`)
          ],
          components: [
            new ActionRowBuilder().addComponents(
              new ButtonBuilder()
              .setStyle(ButtonStyle.Link)
              .setURL("https://discord.gg/kSwwUD8RDe")
              .setLabel("Premium")
            )
          ],
        });
          ...
          if (commandCount > 5) {
            ...
            if (!checkisBlacklisted) {
              ...
              const saixd = new EmbedBuilder()
                ...
                .setDescription(`You have been blacklisted for spamming commands. Please refrain from such behavior\nAstrial Antispam System | No one Can Bypass This!`)
                .addFields({ name: 'Support Server', value: '[Join our support server](https://discord.gg/kSwwUD8RDe)', inline: true })
                .setTimestamp();
    ...
    if (
      ...
      const embed = new EmbedBuilder()
        .setDescription(`<:cross:1301129244855763027> | You don't have enough Permissions !!`)
    ...
    if (
      ...
      const embed = new EmbedBuilder()
        .setDescription(`<:cross:1301129244855763027> | I don't have enough Permissions !!`)
        .setColor(client.config.color);
      return message.reply({ embeds: [embed] });
    }
    if (command.inVc && !message.member.voice.channel) {
      const embed = new EmbedBuilder()
        .setDescription(
          "<:cross:1301129244855763027> | You need to be in a voice channel to use this command!"
        )
        .setColor(client.config.color);
      return message.reply({ embeds: [embed] });
    }
    if (
      ...
      const embed = new EmbedBuilder()
        .setColor(client.config.color)
        .setDescription(`<:cross:1301129244855763027> | I'm not in the same voice channel as you!`);
      return message.reply({ embeds: [embed] });
    }
    ...
    if (command.ownerOnly) {
      ...
      if (!owner) {
        const embed = new EmbedBuilder()
          .setColor(client.color)
          .setDescription(
            "<:cross:1301129244855763027> | It's a owner only command you can't use this command!"
          );
      ...
      try {
        if (!role) {
          const embed = new EmbedBuilder()
            .setColor(client.color)
            .setDescription(
              "<:cross:1301129244855763027> | The DJ role could not be found. It may have been deleted. Please check the server settings."
            );
          return message.channel.send({ embeds: [embed] });
        }
        ...
        if (!member.roles.cache.has(role.id)) {
          const embed = new EmbedBuilder()
            .setColor(client.color)
            .setDescription(
              "<:cross:1301129244855763027> | You do not have the required DJ role to use this command."
            );
          return message.channel.send({ embeds: [embed] });
        }
      } catch (error) {
    ...
    if (restrictionData) {
      if (restrictionData.restrictedTextChannels.includes(message.channel.id)) {
        const embed = new EmbedBuilder()
          .setColor(client.config.color)
          .setDescription("<:warn:1300837192024850473> | This text channel is restricted and you cannot use commands here!");
      ...
      if (message.member.voice.channelId && restrictionData.restrictedVoiceChannels.includes(message.member.voice.channelId)) {
        const embed = new EmbedBuilder()
          .setColor(client.config.color)
          .setDescription("<:warn:1300837192024850473> | You are in a restricted voice channel and cannot use commands!");
