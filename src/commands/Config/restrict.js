...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!type) {
      return message.reply(`${client.emoji.cross} | Please provide a type (e.g., \`text\`, \`voice\`, \`reset\`).`);
    }
    ...
    if (type === "reset") {
      ...
      return message.reply("<:Correct:1301128918614671403> | All channel restrictions have been reset.");
    }
    ...
    if (!channel) {
      return message.reply(`${client.emoji.cross} | Please mention a valid channel or provide a valid channel ID.`);
    }
    ...
    if (type === "text") {
      ...
      if (restrictionData.restrictedTextChannels.includes(channel.id)) {
        return message.reply("${client.emoji.cross} | This text channel is already restricted.");
      }
      restrictionData.restrictedTextChannels.push(channel.id);
      await restrictionData.save();
      return message.reply(`<:Correct:1301128918614671403> | Text channel <#${channel.id}> has been restricted.`);
    } else if (type === "voice") {
      // Restrict Voice Channel
      if (restrictionData.restrictedVoiceChannels.includes(channel.id)) {
        return message.reply("${client.emoji.cross} | This voice channel is already restricted.");
      }
      restrictionData.restrictedVoiceChannels.push(channel.id);
      await restrictionData.save();
      return message.reply(`<:Correct:1301128918614671403> | Voice channel <#${channel.id}> has been restricted.`);
    } else {
      return message.reply("${client.emoji.cross} | Invalid type. Please use `text`, `voice`, or `reset`.");
    }
  },
  ...
  restrict: {
    execute: async (client, interaction) => {
      ...
      if (type === "reset") {
        ...
        if (!restrictionData) {
          restrictionData = new Restriction({ guildId: interaction.guild.id });
        }
        restrictionData.restrictedTextChannels = [];
        restrictionData.restrictedVoiceChannels = [];
        await restrictionData.save();
