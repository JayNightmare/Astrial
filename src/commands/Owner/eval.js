...
module.exports = {
    ...
    run: async (client, message, args, prefix) => {
      let sumant = ['1203569730385084476'];
      if (!sumant.includes(message.author.id)) return;
        else{
            if(!args[0])
            {
                return message.channel.send({embeds : [new EmbedBuilder().setColor(client.color).setDescription(`<:cross:1301129244855763027> | Provide me something to evaluate`)]})
            }
            let ok;
            try{
